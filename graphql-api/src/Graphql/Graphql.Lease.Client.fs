...
module AsOfDateInputDto =
    let toProto (dtoOpt:Types.AsOfDateInputDto option) =
        ...
        let defaultAsOfDate =
            AsOfDate(
                AsAtTime = (now |> DateTime.toProtoTimestamp),
                AsOnDate = (now |> DateTime.toProtoDate))
        dtoOpt
        |> Option.map (fun dto ->
            let asAt =
                dto.AsAt
                |> Option.map (DateTime.Parse >> DateTime.toProtoTimestamp)
                |> Option.defaultValue (now |> DateTime.toProtoTimestamp)
            let asOn =
                dto.AsOn
                |> Option.map (DateTime.Parse >> DateTime.toProtoDate)
                |> Option.defaultValue (now |> DateTime.toProtoDate)
            AsOfDate(
                AsAtTime = asAt,
                AsOnDate = asOn))
